...
public class ChatController {
    ...
    public void chat(@Payload @Valid ChatMessageRequest request) {
        ...
        // 채팅방 리스트 업데이트
        for (ChatMemberResponse chatMember : sendResponse.chatMemberResponseList()) {
            simpMessagingTemplate.convertAndSend("/sub/chat/" + chatMember.id(), chatMember.chatRoomResponse(),
                    headers);
        }
    }
