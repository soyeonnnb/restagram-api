...
public class UserServiceImpl implements UserService {
    ...
    public PaginationResponse searchUser(Long cursorId, String query) {
        ...
        // 유저 리스트 가져오기
        List<User> userList = userRepository.findByIdGreaterThanAndNicknameLike(cursorId, query);
        List<UserInfoResponse> userInfoResponseList = userList.stream().map(UserInfoResponse::of).collect(Collectors.toList());

        // 다음 커서 값 설정
        Long nextCursorId =
                !userInfoResponseList.isEmpty() ? userInfoResponseList.get(userInfoResponseList.size() - 1).id() : null;
