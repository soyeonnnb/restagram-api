...
public class CalendarServiceImpl implements CalendarService {
  ...
  public CalendarAgreeResponse agreeCalendar(Long userId, CalendarAgreeRequest request) {
    ...
    if (!calendarRepository.existsByCustomer(customer)) {
      throw new RestApiException(CalendarErrorCode.CALENDER_NOT_AUTHORIZATION,
          "카카오 캘린더에 동의하지 않았습니다. [회원ID=" + customer.getId() + "]");
    }

    boolean isAgree = customer.updateCalendarAgree(request.agree());
  ...
  private String requestCreateCalender(Customer customer) {
    ...
    if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
      String calenderId = response.getBody().calendar_id();
      return calenderId;
    } else {
